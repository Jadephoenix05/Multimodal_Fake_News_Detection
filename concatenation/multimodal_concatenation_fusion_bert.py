# ============================================================
# MULTIMODAL CONCATENATION BASELINE (BERT + Swin)
...
TEXT_WEIGHTS_PATH = "/kaggle/working/bert_fake_news_model_multiclass.pt"
...
MAX_LENGTH = 80        # BERT tokenization budget (matches text branch)
...
bert_backbone = AutoModel.from_pretrained("bert-base-uncased")
text_model = BertFakeNewsClassifier(bert_backbone, NUM_CLASSES).to(device)
...
torch.save({"features": train_concat, "labels": train_labels},
           os.path.join(OUT_DIR, "train_concat_features_bert.pt"))
torch.save({"features": val_concat, "labels": val_labels},
           os.path.join(OUT_DIR, "val_concat_features_bert.pt"))
torch.save({"features": test_concat, "labels": test_labels},
           os.path.join(OUT_DIR, "test_concat_features_bert.pt"))
...
# ============================================================
# Frozen encoders, frozen concatenation -- only this linear
# layer is trained, matching the paper's controlled comparison
# (fusion strategy is the only thing that varies).
...
torch.save(concat_model.state_dict(),
           os.path.join(OUT_DIR, "concat_classifier_bert.pt"))
print("\nSaved concatenation classifier to",
      os.path.join(OUT_DIR, "concat_classifier_bert.pt"))

print("\n========== SAVED FILES ==========")
print(os.path.join(OUT_DIR, "train_concat_features_bert.pt"))
print(os.path.join(OUT_DIR, "val_concat_features_bert.pt"))
print(os.path.join(OUT_DIR, "test_concat_features_bert.pt"))
print(os.path.join(OUT_DIR, "concat_classifier_bert.pt"))
