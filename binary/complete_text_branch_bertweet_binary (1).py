# ============================================================
# COMPLETE TEXT BRANCH - BERTWEET
# Multimodal Fake News Detection Project
# Binary Classification: 0 = Real, 1 = Fake
# (matches complete_image_branch_swin.py's label convention --
#  original label 0 -> Real (0), any other original label -> Fake (1))
#
...
# 3. Loads BERTweet (vinai/bertweet-base)
# 4. Fine-tunes BERTweet for Fake/Real classification
...
# ============================================================
# 2. INSTALL TRANSFORMERS + EMOJI
# ============================================================
# BERTweet's tokenizer normalization step needs the `emoji`
# package to convert emojis into text tokens (:smiling_face:).
...
# ============================================================
# 5b. COLLAPSE FINE-GRAINED LABELS TO BINARY
# ============================================================
# Your raw label column may follow a fine-grained taxonomy
# (e.g. True/Satire/Misleading/False Connection/Manipulated/
# Imposter, coded 0-5). To match the image branch's binary
# Real/Fake setup:
#   original label == 0  -> 0 (Real)
#   original label != 0  -> 1 (Fake)
# Edit this rule if your "Real" class isn't coded as 0.

print("\nOriginal label distribution:")
print(available["label"].value_counts().sort_index())

available["label"] = (available["label"] != 0).astype(int)

NUM_CLASSES = 2

print("\nBinary label distribution (0=Real, 1=Fake):")
print(available["label"].value_counts().sort_index())
...
# ============================================================
# 12. DEFINE BERTWEET FAKE/REAL CLASSIFIER
# ============================================================
...
class BertweetFakeNewsClassifier(nn.Module):

    def __init__(self, text_model, num_classes):
        super().__init__()
        self.bertweet = text_model
...
model = BertweetFakeNewsClassifier(bertweet, NUM_CLASSES).to(device)
...
# ============================================================
# We fine-tune the complete BERTweet encoder, mirroring the
# image branch's approach (full fine-tune, not frozen).

for param in model.bertweet.parameters():
...
optimizer = torch.optim.AdamW(
    [
        {"params": model.bertweet.parameters(), "lr": 2e-5},
...
#
# Text -> BERTweet -> 768-dimensional [CLS] feature vector
#
...
torch.save(
    ...
    "/kaggle/working/train_text_features_binary.pt"
)

torch.save(
    {"features": val_features, "labels": val_labels},
    "/kaggle/working/val_text_features_binary.pt"
)

torch.save(
    {"features": test_features, "labels": test_labels},
    "/kaggle/working/test_text_features_binary.pt"
)
...
torch.save(
    model.state_dict(),
    "/kaggle/working/bertweet_fake_news_model_binary.pt"
)

print("Trained BERTweet model saved successfully!")
...
print("\n========== SAVED FILES ==========")
print("/kaggle/working/train_text_features_binary.pt")
print("/kaggle/working/val_text_features_binary.pt")
print("/kaggle/working/test_text_features_binary.pt")
print("/kaggle/working/bertweet_fake_news_model_binary.pt")
